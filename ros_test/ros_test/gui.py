...
class CameraSubscriber(Node):
    def __init__(self, image_signal: ImageSignal):
        ...
        self.create_subscription(Image, '/lights/channel_1_frame', self.callback_factory('front'), 10)
        self.create_subscription(Image, '/back_camera/image_raw', self.callback_factory('back'), 10)
        self.create_subscription(Image, '/left_camera/image_raw', self.callback_factory('left'), 10)
        self.create_subscription(Image, '/right_camera/image_raw', self.callback_factory('right'), 10)
    ...
    def callback_factory(self, key):
        ...
        def callback(msg):
            try:
                ...
                bytes_per_line = ch * w     #Calculates bytes of one row, required for QImage
...
class MainWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        sensor_placeholder.setStyleSheet("color: gray; font-size: 16px; margin-top: 20px;")
